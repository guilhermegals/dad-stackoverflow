...
class LoginRepositoryImpl @Inject constructor(
    ...
    override suspend fun login(request: LoginRequest): ResponseModel<String> =
        withContext(dispatcher) {
            try {
                val response = service.login(request)
                val resultBody = response.body()

                if (response.isSuccessful && resultBody != null && !resultBody.token.isNullOrBlank()) {
                    sharedPreferencesRepository.setAccessToken(resultBody.token)
